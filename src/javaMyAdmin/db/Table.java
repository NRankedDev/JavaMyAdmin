...
public class Table {
	...
	public void setValue(int line, int column, String value) throws SQLException {
		...
		connect.createStatement()
				.executeUpdate("UPDATE `" + dbname + "`.`" + getName() + "` SET `" + getColumnNames(column) + "` = '" + value + "' WHERE `" + getColumnNames(0) + "` = " + getLines(line).getValues(0));
	}
	...
	public String getColumnInfo(String column, int i) throws SQLException {
		...
		return (rs = connect.createStatement().executeQuery("select `" + value + "` from information_schema.columns where table_name='" + name + "' and column_name like '" + column + "'")).next()
				? rs.getString(1) : null;
	}
